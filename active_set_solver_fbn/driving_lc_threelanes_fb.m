...
global speed_pen lat_speed_pen polite_pen;
...
function const = left_lane_ineq(x)
    global lane_width half_w;
    const = [x(1)-1.9*half_w+(2*lane_width)];
end
...
function ownerships = assign_running_ineq_ownerships()
    ownerships{1} = [1];  % a and b
    ownerships{2} = [3];  % a and c
    ownerships{3} = [1];  % a and d
    ownerships{4} = 1;
    ownerships{5} = 1;
    ownerships{6} = 2;
    ownerships{7} = 2;
    ...
    ownerships{10} = 4;
    ownerships{11} = 4;
    ownerships{12} = 2; % b and c
    ownerships{13} = 2; % b and d
    ownerships{14} = 3; % c and d
...
function val = running_cost_a1_polite(xu)
   ...
   val = running_cost_a1(xu) + ...
          polite_pen*running_cost_b1(xu) + ...
          polite_pen*running_cost_c1(xu) + ...
          polite_pen*running_cost_d1(xu);
end
