...
x0 = [border_right/2; 0;0;  gv_a1;
      border_left+border_left/2; -18;0; gv_b1;%1
...
function plot_solution(xval,x0)
    ...
    if make_vid
        writerObj = VideoWriter('three_lane_driving_standard_with_plans','MPEG-4');
...
%         color_c = [1,.7,0];
        ...
        car_d1 = draw_car(xx(t,13:14),color_d,1,pi/2);
        
        for k = 1:T/5    
            y = xval{t+k};
            spot_a1{k} = viscircles(y(1:2)',0.25,'Color', color_a,'LineStyle','-');
            spot_b1{k} = viscircles(y(5:6)',0.25,'Color', color_b,'LineStyle','-');
            spot_c1{k} = viscircles(y(9:10)',0.25,'Color', color_c,'LineStyle','-');
            spot_d1{k} = viscircles(y(13:14)',0.25,'Color', color_d,'LineStyle','-');
        end
