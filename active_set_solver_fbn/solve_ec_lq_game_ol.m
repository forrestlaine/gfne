...
function [K, k] = solve_ec_lq_game_ol(F,... % time-indexed dict of dynamics
    ...
    for t = T:-1:1
        ...
        for i = 1:N
            z2modz{i} = z2mod;
            z2modz{i}(n+1+1:n+1+m_start{i},:) = zeros(m_start{i},sz2);
            z2modz{i}(n+1+m_start{i}+m{i}+1:end,:) = zeros(all_m-m_start{i}-m{i},sz2);
        end
        
        
        Nall = [];
        for i = 1:N
            Qtz{i} = z2modz{i}'*Qt{i}*z2modz{i};
        ...
        for i = 1:N
            upz{i} = upd;
            upz{i}(n+1+1:n+1+m_start{i},:) = zeros(m_start{i},n+1);
            upz{i}(n+1+m_start{i}+m{i}+1:end,:) = zeros(all_m-m_start{i}-m{i},n+1);
        end
        for i = 1:N
            P{i} = upz{i}'*Qt{i}*upz{i};
            G{i} = Rt{i}*upz{i};
        end
    end
end