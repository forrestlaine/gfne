...
buffer_behind_length_a = 1;
buffer_behind_length_b = 1;
buffer_behind_length_c = 1;
buffer_front_tip = 3;
buffer_width_wide = 2.0;
buffer_width_narrow = 1.95;
...
x0 = [border_right/2; -10;0;  gv_a1;
      border_left/2; -28;0; gv_b1;%1
      border_left/2; 8;0;  gv_b1;%2
      border_right/2; 25;0; gv_d1];%3
...
function plot_solution(xval,x0)
    ...
    if make_vid
        writerObj = VideoWriter('three_lane_shared_merge','MPEG-4');
...
%         color_c = [1,.7,0];
        ...
        car_c1 = draw_car(xx(t,9:10),color_b,1,pi/2);
...
function const = running_ineq_constraint_a12(x)
    % collision zone of xd
    global buffer_behind_length_b buffer_behind_length_c;
    xa1 = x(1:2);
    xb1 = x(5:6);
    xc1 = x(9:10);
    xd1 = x(13:14);
    const = avoid_polygon(xb1,xc1,buffer_behind_length_c, buffer_behind_length_b);
end

function const = running_ineq_constraint_a13(x)
    % collision zone of xd
    global buffer_behind_length_b buffer_behind_length_c;
    xa1 = x(1:2);
    xb1 = x(5:6);
    xc1 = x(9:10);
    xd1 = x(13:14);
    const = avoid_polygon(xb1,xd1,buffer_behind_length_c, buffer_behind_length_b);
end

function const = running_ineq_constraint_a14(x)
    % collision zone of xd
    global buffer_behind_length_b buffer_behind_length_c;
    xa1 = x(1:2);
    xb1 = x(5:6);
    xc1 = x(9:10);
    xd1 = x(13:14);
    const = avoid_polygon(xc1,xd1,buffer_behind_length_c, buffer_behind_length_b);
end
...
function const = left_lane_ineq(x)
    global lane_width half_w;
    const = [x(1)-1.98*half_w+(2*lane_width)];
end

function const = right_lane_ineq(x)
    global lane_width half_w;
    const = [-x(1)-1.98*half_w+(lane_width)];
end
...
function const = assign_running_ineq_constraints()
    ...
    const{4} = @running_ineq_constraint_a4; 
    const{5} = @running_ineq_constraint_a5;
    ...
    const{12} = @running_ineq_constraint_a12;
    const{13} = @running_ineq_constraint_a13;
    const{14} = @running_ineq_constraint_a14;
end

function ownerships = assign_running_ineq_ownerships()
    ownerships{1} = [1,2];  % a and b
    ownerships{2} = [1,3];  % a and c
    ownerships{3} = [1,4];  % a and d
    ownerships{4} = 1;
    ownerships{5} = 1;
    ownerships{6} = 2;
    ownerships{7} = 2;
    ...
    ownerships{10} = 4;
    ownerships{11} = 4;
    ownerships{12} = [2,3]; % b and c
    ownerships{13} = [2,4]; % b and d
    ownerships{14} = [3,4]; % c and d
end
...
function draw_lanes()
    global border_left border_right;
    plot([2*border_left, 2*border_left],[-200,10000],'-k');
