...
for t = 1:T
    ...
    lf{t,6} = @running_cost_cheat;
    lf{t,7} = @running_cost_limits_left;
    lf{t,8} = @running_cost_limits_right;
...
lf{T+1,6} = @empty_cost;
lf{T+1,7} = @terminal_cost_limits_left;
lf{T+1,8} = @terminal_cost_limits_right;
...
gf{T+1} = @terminal_ineq_constraint;
...
[xval,ws,durs] = fb_game_solver_active_set_shared(@f, h, hi, gf, gi, gr, lf, n, m, N, T, x0, rollout_x0, params, @plot_solution);
solution_set{1} = xval;
durations{1} = durs;
