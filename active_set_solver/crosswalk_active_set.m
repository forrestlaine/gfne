...
prob_b1 = 0.5;
params.cool = true;
xval = fb_game_solver_active_set(@f, h, g, l, n, m, N, T, x0, rollout_x0, params, @plot_solution);
solution_set{1} = xval;
...
%         axis tight
    ...
    for t = 1:T+1
        for s = L:-1:1
            ...
            spot_a1{s} = viscircles(xx(t,1:2),ped_r,'Color', alphas(s)*[0,0,1],'LineStyle',':');
            ped_a1{s} = draw_ped(xx(t,1:2),alphas(s)*[0,0,1]);

%             spot_b1{s} = viscircles(xx(t,5:6),car_r,'Color', 'g','LineStyle',':');
%             car_b1{s} = draw_car(xx(t,5:6),'r');

            spot_b2{s} = viscircles(xx(t,9:10),car_r,'Color', alphas(s)*[1,0,0],'LineStyle',':');
            car_b2{s} = draw_car(xx(t,9:10),alphas(s)*[1,0,0]);
...
function const = running_ineq_constraint_b1(x)
    ...
    const = [sqrt((xb1-xa1)'*(xb1-xa1))-avoid_r];
...
function const = running_ineq_constraint_b2(x)
    ...
    const = [sqrt((xb2-xa1)'*(xb2-xa1))-avoid_r];
...
function val = running_cost_a1(xu)
    ...
    xa1 = xu(1:4);
    ua1 = xu(13:14);
    val = 1*ua1'*ua1 + speed_pen*(xa1(3)-gv_a1)^2+crosswalk_cost(xa1);
end
...
function val = running_cost_a1_polite(xu)
    global prob_b1;
    val = running_cost_a1(xu)+0.2*(prob_b1*running_cost_b1(xu)+(1-prob_b1)*running_cost_b2(xu));
end
