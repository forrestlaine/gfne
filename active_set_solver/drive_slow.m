...
x0 = [border_left/2; -15;0; gv_a1;
      border_right/2; 0;0;gv_b1;
      border_right/2; 0;0;gv_b2;
      border_right/2; 13;0;gv_c1];
  

rollout_x0 = [border_left/2; -15; 0; gv_b1;
      border_right/2;  0;0;gv_b1;
      border_right/2; 0;-0.1;gv_b1+1;
      border_right/2; 13;0;gv_c1];
...
% [xval,ws,duration] = fb_game_solver_active_set(@f, h, g, l, n, m, N, T, x0, rollout_x0, params, @plot_solution,ws,xval);
% solution_set{2} = xval;
...
plot_solution_set({solution_set{5},solution_set{3},solution_set{1}});
...
function plot_solution(xval,x0)
    ...
    for t = 1:T+1
        ...
        spot_b1 = viscircles(xx(t,5:6),car_r,'Color', 'r','LineStyle',':');
        car_b1 = draw_car(xx(t,5:6),'r',1);
...
function plot_solution_set(solution_set)
    ...
    close all;
%     fh = figure; 
    fh = figure('Position', [300 300 250 450]); 
    if make_vid
        writerObj = VideoWriter('drive_slow_spedup','MPEG-4');
...
function const = running_ineq_constraint_b1(x)
    ...
    const = [avoid_constraint(xc1,xb1)];
...
function const = running_ineq_constraint_b2(x)
    ...
    xb2 = x(9:10);
    xc1 = x(13:14);
    const = [avoid_constraint(xa1,xb2); left_lane_constraint(xb2)+0.1];
%     const = zeros(0,1);
...
function val = running_cost_a1(xu)
    ...
    xa1 = xu(1:4);
    ua1 = xu(17:18);
    val = 100*ua1'*ua1 + speed_pen*speed_cost(xa1,gv_a1);
end
...
function val = running_cost_b1(xu)
    ...
    xb1 = xu(5:8);
    ub1 = xu(19:20);
    val = ub1'*R*ub1 + speed_pen*speed_cost(xb1,gv_b1);
end
...
function val = running_cost_b2(xu)
    ...
    xb2 = xu(9:12);
    ub2 = xu(21:22);
    val = ub2'*R*ub2 + speed_pen*speed_cost(xb2,gv_b2)+0*left_lane_cost(xb2);
end
...
function val = running_cost_a1_polite(xu)
    global prob_b1;
    prob_b2 = 1-prob_b1;
    val = running_cost_a1(xu)+ prob_b1/(1-prob_b1)*running_cost_b1(xu) + prob_b2/(1-prob_b2)*running_cost_b2(xu);
end
