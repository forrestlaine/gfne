...
function [dX,dU,dL,dM,dP,Kk] = solve_ec_lq_game_super_fast_mod(F,... % time-indexed dict of dynamics
...
%     l = cell(T,N);
	...
	for t = T:-1:1
        ...
        for i = 1:N
            ...
            Hu = H{t,i}(:,n+2+start_m{i}:n+1+start_m{i}+m{i});
            HHu = [G{i}(:,2:end)*B; Hu];
            rk = rank(HHu);
        ...
        if all_feas{t}
            if over_constrained
                ...
                [G,P,KKt] = solve_over_constrained_step_mod(Ft,Ht,G,Qt,P,N,m);
                Kk{t} = KKt;
                ...
                [~,~,LAM,MM,~] = solve_ec_lq_game_dpol_mod(FF,HH,QQ,N,S,KK,m);
                ...
                [G,P,KKt,LLt,Lampolt] = solve_under_constrained_step_mod(Ft,Ht,G,Qt,P,N,m,all_m);
                Kk{t} = KKt;
                for i = 1:N
                    Ll{t,i} = LLt{i}(1:size(Ht{i},1),:);
                    if prod(size(G{i})) > 0 || t==T
                        Lid{t+1} = t;
