...
global prob_b1 prob_b2 prob_c1;
...
% disp('done');
% plot_solution(xval,0);
plot_solution_set({solution_set{1},solution_set{2},solution_set{4}});
...
function plot_solution_set(solution_set)
    ...
    if make_vid
        writerObj = VideoWriter('double_lane_change','MPEG-4');
        writerObj.FrameRate = 10;
        open(writerObj);
%         axis tight
        set(gca,'nextplot','replacechildren');
...
%             car_b1{s} = draw_car(xx(t,5:6),[1,.7,0],alphas(s),ls(s));
%         end
        for s = 1:L
            xx(t,:) = solution_set{s}{t};
%             spot_b2{s} = viscircles(xx(t,9:10),car_r,'Color', 'r','LineStyle',':');
            car_b2{s} = draw_car(xx(t,9:10),[1,0,0],alphas(s),ls{s});
            car_b1{s} = draw_car(xx(t,5:6),[1,0,0],alphas(s),ls{s});
        end
        for s = 1:L
            xx(t,:) = solution_set{s}{t};
            car_a1{s} = draw_car(xx(t,1:2),[0,0,1],alphas(s),ls{s});
            car_c1{s} = draw_car(xx(t,13:14),'g',alphas(s),ls{s});
            car_c2{s} = draw_car(xx(1,13:14)+(t-1)*dt*[0,gv_c1],'g',alphas(s),ls{s});
        end

        y = solution_set{1}{t}(2);
...
function val = running_cost_a1_ll(xu)
    ...
    xa1 = xu(1:4);
    ua1 = xu(17:18);
    val = ua1'*R*ua1 + speed_pen*speed_cost(xa1,gv_a1) + lane_pen*left_lane_cost(xa1);
end
...
function val = running_cost_b2(xu)
    ...
    xb2 = xu(9:12);
    ub2 = xu(21:22);
    val = ub2'*R*ub2 + speed_pen*speed_cost(xb2,gv_b2) + lane_pen*left_lane_cost(xb2);
end
...
function val = running_cost_c1_ll(xu)
    global gv_c1;
    global speed_pen lane_pen;
    global R;
    xc1 = xu(13:16);
    uc1 = xu(23:24);
    val = uc1'*R*uc1 + speed_pen*speed_cost(xc1,gv_c1) + lane_pen*left_lane_cost(xc1);
end
...
function val = running_cost_a1_polite(xu)
    global prob_b1;
    global prob_b2 prob_c1;
    
    r1 = prob_b1/(1-prob_b1);
    r2 = prob_b2/(1-prob_b2);
    r3 = prob_c1/(1-prob_c1);
    
    val = running_cost_a1(xu) + ...
        r1*running_cost_b1(xu) +...
        r2*running_cost_b2(xu) +...
        r3*running_cost_c1(xu);
end
...
function val = running_cost_a1_polite_ll(xu)
    ...
    r2 = prob_b2/(1-prob_b2);
    r3 = prob_c1/(1-prob_c1);
    val = running_cost_a1_ll(xu) +...
        r1*running_cost_b1(xu) +...
        r2*running_cost_b2(xu) +...
        r3*running_cost_c1_ll(xu);
%     
...
function pt = draw_car(center,color,alpha,ls)
    ...
    pt = patch('XData',[center(1)-half_w,center(1)-half_w,center(1)+half_w,center(1)+half_w,center(1)-half_w],...
               ...
               'FaceAlpha',alpha,'FaceColor', color,'LineStyle',ls,'EdgeColor',color,'LineWidth',2);
end
...
function draw_lanes()
    global border_left border_right;
    plot([border_left-4, border_left-4],[-200,200],'-k');
