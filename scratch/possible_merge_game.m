...
x0 = [border_left/2;-5;0;0;
      border_right/2;15;0;0;
       border_right/2-0.01;5;0;0;
       border_right/2-0.01;5;0;0];

global goal_a goal_b goal_c1 goal_c2;
goal_a = [border_left/2;  15];
goal_b = [border_right/2;  15];
goal_c1 = [border_left/2;  5];
goal_c2 = [border_right/2;25];
...
params.tauval_decrease = 1e-1;
params.tauval_tolerance = 1e-4;
params.resid_tolerance = 1e-4;
params.debug_plot = false;
params.max_linesearch_iters = 5;
params.alpha_init = 1;
...
function const = running_ineq_constraint_a(x)
    ...
    const = [const;
             border_right+1-half_w-x(1);
             x(1)+1-border_left-half_w];
end
...
function const = running_ineq_constraint_c1(x)
    ...
    xc1 = x(9:10);
    xc2 = x(13:14);
    const = [(xc1-xb)'*(xc1-xb)-avoid_r*avoid_r];
    const = [const;
...
function const = running_ineq_constraint_c2(x)
    ...
    xc1 = x(9:10);
    xc2 = x(13:14);
    const = [(xc2-xb)'*(xc2-xb)-avoid_r*avoid_r];
    const = [const;
            border_right-half_w-x(13);
