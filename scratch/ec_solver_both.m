...
function [residuals, xval] = ec_solver_both(f,... % dynamics function
    ...
    for t = 1:T
        ...
        for i = 1:N
            ...
            all_vars = [all_vars; u{t,i}; mu{t,i}; lam{t,i}];
        ...
        for i = 1:N
            ...
            lagrangian{i} = lagrangian{i} + cost{t,i} + mu{t,i}'*constraint{t,i} + lam{t,i}'*(pred{t}-x{t+1});
        end
...
%     %% initialize policies
    [K, k] = solve_ec_lq_game_r(F,H,Q,N,T);
%     for t = 1:T
%         KKK{t} = [];
%         kkk{t} = [];
%         for i = 1:N
...
%     end
    ...
    for iter = 1:100 % max iterations
        ...
        [K, k] = solve_ec_lq_game_r(F,H,Q,N,T);
        [dX,dU,dL,dM,dP1] = solve_ec_lq_game_d(F,H,Q,N,T,K,zeros(size(z0)));
        [dX2,dU2,dL2,dM2,dP,~,~] = solve_ec_lq_game_both(F,H,Q,N,T,K,k,zeros(size(z0)));
...
%                         c_psival{t,i}(1:all_m-m{i},1) = psival{t,i}(1:all_m-m{i})+alpha*(dP{t,i}(1:all_m-m{i})-psival{t,i}(1:all_m-m{i}));
%                         c_psival{t,i}(all_m-m{i}+1:all_m,1) = psival{t,i}(all_m-m{i}+1:all_m);
                    end
                end
                ...
                for i = 1:N
                    ...
                    hess(1+1:1+n,1+1:1+n) = hess(1+1:1+n,1+1:1+n)+HK{t,i};
...
%             c_K = K;
%             if ~open_loop
%             [c_K, ~] = solve_ec_lq_game_r(c_F,c_H,c_Q,N,T);
%             [~,~,~,~,~,c_W,c_w] = solve_ec_lq_game_both(F,H,Q,N,T,K,k,zeros(size(z0)));
            [c_K, c_k] = solve_ec_lq_game_r(c_F,c_H,c_Q,N,T);
%             c_W = W;
%             end
            ...
            for i = 1:N
                ...
                for t = 1:T-1
                    ...
                    c_KK{t+1,i} = [c_KK{t+1,i}; c_K{t+1,i}];
                    state_opt(ind+1:ind+n) = state_opt(ind+1:ind+n) + c_KK{t+1,i}'*c_psival{t,i};
                    other_control_opt(ind2+1:ind2+all_m-m{i}) = other_control_opt(ind2+1:ind2+all_m-m{i}) - c_psival{t,i}(1:all_m-m{i});
                    control_opt(ind3+1:ind3+m{i}) = control_opt(ind3+1:ind3+m{i}) - c_psival{t,i}(all_m-m{i}+1:end);
                    ind = ind+n;
                    ind2 = ind2+all_m-m{i};
                    ind3 = ind3+m{i};
                end
            ...
            if true || (new_current_residual < current_residual)
