...
x0 = [border_right/2; 5;0; gv_a1;
      border_left/2; -8;0;gv_b1;
      border_right/2; 0;0;gv_b2];
  

rollout_x0 = [border_right/2; 5;-.1; gv_a1;
      border_left/2; -8;0;gv_b2;
      border_right/2; 0;0;gv_b2];
...
params.wait_for_policy_to_update_tau = false;
params.fresh_policy_threshold = 1;
...
prob_b1 = 0.25;
[residuals, xval, initialization2] = ec_solver_clean(@f, h, g, l, n, m, N, T, x0, params, @plot_solution,initialization);
solution_set{2} = xval;
...
%         axis tight
    ...
    for t = 1:T+1
        for s = 1:L
            ...
            spot_a1{s} = viscircles(xx(t,1:2),car_r,'Color', 'b','LineStyle',':');
            car_a1{s} = draw_car(xx(t,1:2),'b');
        ...
        axis([-20,+20,xx(t,10)-20,xx(t,10)+20]);
        axis('equal');
        if make_vid
            frame = getframe(fh);
            writeVideo(writerObj,frame);
...
function val = running_cost_a1(xu)
    ...
    xa1 = xu(1:4);
    ua1 = xu(13:14);
    val = .75*ua1'*ua1 + 0*speed_cost(xa1,gv_a1) + 0.1*left_lane_cost(xa1);
end
...
function val = running_cost_b2(xu)
    ...
    xb2 = xu(9:12);
    ub2 = xu(17:18);
    val = ub2'*R*ub2 + speed_pen*speed_cost(xb2,gv_b2);
end
...
function val = running_cost_a1_polite(xu)
    global prob_b1;
    val = running_cost_a1(xu)+prob_b1*running_cost_b1(xu);
end
...
function const = avoid_constraint(x1,x2)
    global avoid_r;
    delt = x1(1:2)-x2(1:2);
    const = delt'*delt-avoid_r*avoid_r;
end
