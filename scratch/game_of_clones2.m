...
x0 = [border_left/2;-20;0;gv_c1;
      border_left/2;-20;0;gv_c1;
       border_right/2;-10;0;gv_c1;
       border_right/2;-10;0;gv_c1;
       border_right/2;10;0;gv_c1;
       border_right/2;10;0;gv_c1];
...
function plot_solution(xval,x0,varargin)
    ...
    for t = 1:T+1
        ...
        spot_a2 = viscircles(xx(t,5:6),avoid_r/2,'Color', 'b','LineStyle',':');
        car_a2 = draw_car(xx(t,5:6),'b');

        spot_b1 = viscircles(xx(t,9:10),avoid_r/2,'Color', 'r');
        car_b1 = draw_car(xx(t,9:10),'r');
        spot_b2 = viscircles(xx(t,13:14),avoid_r/2,'Color', 'r','LineStyle',':');
        car_b2 = draw_car(xx(t,13:14),'r');

        spot_c1 = viscircles(xx(t,17:18),avoid_r/2,'Color', 'g');
        car_c1 = draw_car(xx(t,17:18),'g');
        spot_c2 = viscircles(xx(t,21:22),avoid_r/2,'Color', 'g','LineStyle',':');
        car_c2 = draw_car(xx(t,21:22),'g');
...
%     delete(car_c2);
    ...
    ind = 1;
    for s = 1:length(solution_set)
        if s==length(solution_set)
            term = T+1;
        else
            term = 19;
        end
        for t = 1:term
            xx(ind,:) = solution_set{s}{t};

            spot_a1 = viscircles(xx(ind,1:2),avoid_r/2,'Color', 'b');
            car_a1 = draw_car(xx(ind,1:2),'b');
%             spot_a2 = viscircles(xx(ind,5:6),avoid_r/2,'Color', 'b','LineStyle',':');
%             car_a2 = draw_car(xx(ind,5:6),'b');

            spot_b1 = viscircles(xx(ind,9:10),avoid_r/2,'Color', 'r');
            car_b1 = draw_car(xx(ind,9:10),'r');
%             spot_b2 = viscircles(xx(ind,13:14),avoid_r/2,'Color', 'r','LineStyle',':');
%             car_b2 = draw_car(xx(ind,13:14),'r');

            spot_c1 = viscircles(xx(ind,17:18),avoid_r/2,'Color', 'g');
            car_c1 = draw_car(xx(ind,17:18),'g');
%             spot_c2 = viscircles(xx(ind,21:22),avoid_r/2,'Color', 'g','LineStyle',':');
%             car_c2 = draw_car(xx(ind,21:22),'g');
            
            
            axis([xx(ind,9)-20,xx(ind,9)+20,xx(ind,10)-20,xx(ind,10)+20]);
            axis('equal');
            if make_vid
                frame = getframe(fh);
                writeVideo(writerObj,frame);
...
function const = running_ineq_constraint_a1(x)
    ...
    const = [(xa1-xb1)'*(xa1-xb1)-avoid_r*avoid_r;
             (xa1-xc1)'*(xa1-xc1)-avoid_r*avoid_r];
    const = [const;
...
function const = running_ineq_constraint_a2(x)
    ...
    const = [(xa2-xb2)'*(xa2-xb2)-avoid_r*avoid_r;
             (xa2-xc2)'*(xa2-xc2)-avoid_r*avoid_r];
    const = [const;
             right_lane_ineq_constraint(xa2);
             left_lane_ineq_constraint(xa2)];
end
...
function const = running_ineq_constraint_b1(x)
    ...
    const = [(xb1-xc1)'*(xb1-xc1)-avoid_r*avoid_r
             (xb1-xa1)'*(xb1-xa1)-avoid_r*avoid_r];
    const = [const;
...
function const = running_ineq_constraint_b2(x)
    ...
    const = [(xb2-xc2)'*(xb2-xc2)-avoid_r*avoid_r;
             (xb2-xa2)'*(xb2-xa2)-avoid_r*avoid_r];
    const = [const;
...
function const = running_ineq_constraint_c2(x)
    ...
    xc2 = x(21:22);
    const = zeros(0,1);
    const = [(xc2-xb2)'*(xc2-xb2)-avoid_r*avoid_r;
...
function const = terminal_constraint_a2(x)
    xa2 = x(5:6);
    const = right_lane_constraint(xa2);
end
...
function val = running_cost_a2(xu)
    ...
    xa2 = xu(5:8);
    ua2 = xu(27:28);
    val = ua2'*ua2 + speed_pen*speed_cost(xa2,gv_a2)+center_lane_pen*center_lane_cost(xa2);
end
...
function val = running_cost_b2(xu)
    ...
    xb2 = xu(13:16);
    ub2 = xu(31:32);
    val = ub2'*ub2 + speed_pen*speed_cost(xb2,gv_b2)+center_lane_pen*center_lane_cost(xb2);
end
...
function val = running_cost_a1_polite(xu)
    global prob_c1;
    val = running_cost_a1(xu)+running_cost_b1(xu)+running_cost_c1(xu);
end

function val = running_cost_a2_polite(xu)
    global prob_c1;
    val = running_cost_a2(xu)+running_cost_b2(xu)+running_cost_c2(xu);
end

function val = running_cost_b1_polite(xu)
    global prob_a1;
    val = running_cost_b1(xu) + running_cost_a1(xu) + running_cost_c1(xu);
end

function val = running_cost_b2_polite(xu)
    global prob_a1;
    val = running_cost_b2(xu) + running_cost_a2(xu) + running_cost_c2(xu);
end

function val = running_cost_c1_polite(xu)
    val = running_cost_c1(xu)+running_cost_b1(xu)+running_cost_a1(xu);
end

function val = running_cost_c2_polite(xu)
    val = running_cost_c2(xu)+running_cost_b2(xu)+running_cost_a2(xu);
end
%% Helpers
