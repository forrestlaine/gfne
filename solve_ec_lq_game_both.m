...
function [dX,dU,dL,dM,dP,WW,ww] = solve_ec_lq_game_both(F,... % time-indexed dict of dynamics
...
% 
    ...
    for t = T-1:-1:1
        ...
        for i = 1:N
            ...
            top_block(ind+1:ind+n,ind2-n-(N)*all_m+1+ind4:ind2-n-(N)*all_m+all_m+ind4) = KK';
            ...
            bleft_block(ind3+1:ind3+m{i},end-n-(N-i)*all_m+1-m{i}:end-n-(N-i)*all_m) = -eye(m{i});
        ...
        top_block(ind+1:ind+all_m,ind2-n+1:ind2) = KK;
        top_block(ind+1:ind+all_m,ind2+1:ind2+all_m) = -eye(all_m);        
        top_vec(ind+1:ind+all_m) = kk;
