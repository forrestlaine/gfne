...
for s = 1:S
    ...
    for t = 1:T
        ...
        vars_a{s} = [vars_a{s}; U_a{s,t}; X_a{s,t}];
        vars_b{s} = [vars_b{s}; U_b{s,t}; X_b{s,t}];
...
lam_a_all = MX.sym('lam_a', size(h_a_all,1));
lam_b_all = MX.sym('lam_a', size(h_b_all,1));

for s = 1:S
    if (s < S)
        g_a{s} = h_a{s+1}(1:4);
        g_b{s} = h_b{s+1}(1:4);
        gam_a{s} = MX.sym(['gam_a_' num2str(s)], 4);
        gam_b{s} = MX.sym(['gam_b_' num2str(s)], 4);
    end
    lam_a{s} = MX.sym(['lam_a_' num2str(s)], size(h_a{s},1));
    lam_b{s} = MX.sym(['lam_b_' num2str(s)], size(h_b{s},1));
end

% Set up necessary condition vector 
for s = 1:S
    Lag_a{s} = f_a{s} - lam_a{s}'*h_a{s};
    Lag_b{s} = f_b{s} - lam_b{s}'*h_b{s};
    
    if (s < S)
        Lag_a{s} = Lag_a{s} - gam_a{s}'*g_a{s};
        Lag_b{s} = Lag_b{s} - gam_b{s}'*g_b{s};
    end
...
for s = 1:S
    pL{s} = [gradient(f_a{s} - lam_a{s}'*h_a{s}, vars_a{s});
             gradient(f_b{s} - lam_b{s}'*h_b{s}, vars_b{s});
             h_a{s};
             h_b{s}];
    sL{s} = [gradient(f_b{s} - lam_b{s}'*h_b{s}, vars_a{s});
             gradient(f_a{s} - lam_a{s}'*h_a{s}, vars_b{s});
             h_a{s};
             h_b{s}];
    stage_vars{s} = [vars_a{s}; vars_b{s}; lam_a{s}; lam_b{s}];
    sys_inv{s} = inv(jacobian(pL{s},stage_vars{s}));
%     s_sys_inv{s} = inv(jacobian(sL{s},stage_vars{s}));
    na = size(vars_a{s},1);
    nb = size(vars_b{s},1);
    la = size(lam_a{s},1);
    lb = size(lam_b{s},1);
    d_vars_aa_dh{s} = sys_inv{s}(1:na, na+nb+1:na+nb+la);
...
all_vars = [];
for s = 1:S
    all_vars = [all_vars; vars_a{s}];
end
for s = 1:S
    all_vars = [all_vars; vars_b{s}];
end
...
%     d_vars_a_dh{s} = preHHi(ind+1:ind+n,jnd+1:jnd+4);
    if (s < S)
        func = f_a{s}-gam_a{s}'*g_a{s};
    else
        func = f_a{s};
    end
    traj_grad_a{s} = gradient(func,vars_a{s});
...
%     d_vars_b_dh{s} = preHHi(ind+1:ind+n,jnd+1:jnd+4);
    if (s < S)
        func = f_b{s}-gam_b{s}'*g_b{s};
    else
        func = f_b{s};
    end
    traj_grad_b{s} = gradient(func,vars_b{s});
...
for s = 1:S-1
    if closed_loop
        ...
        gL = [gL; 
            gam_a{s}-d_vars_aa_dh{s+1}'*traj_grad_a{s+1}; 
            gam_b{s}-d_vars_bb_dh{s+1}'*traj_grad_b{s+1}];        
    end
end
...
for s = 1:S
    ...
    dvars_ab{s} = Function(['extract_dvars_ab_' num2str(s)], {all_vars}, {d_vars_ab_dh{s}});
    dvars_ba{s} = Function(['extract_dvars_ba_' num2str(s)], {all_vars}, {d_vars_ba_dh{s}});
    sgrad_a{s} = Function(['extract_shadow_a_' num2str(s)], {all_vars}, {shadow_grad_a{s}});
    sgrad_b{s} = Function(['extract_shadow_b_' num2str(s)], {all_vars}, {shadow_grad_b{s}});
end
