...
for k = 1:K
    ...
    for t = 1:T
        ...
        vars_a{k} = [vars_a{k}; U_a{t}; X_a{t+1}];
        vars_b{k} = [vars_b{k}; U_b{t}; X_b{t+1}];
        ...
        g_a = [g_a; 
            (X_a{t+1}(1:2))'*(X_a{t+1}(1:2)) - 9;
            (X_a{t+1}(1:2)-X_b{t+1}(1:2))'*(X_a{t+1}(1:2)-X_b{t+1}(1:2)) - 4];
        g_b = [g_b; 
            (X_b{t+1}(1:2))'*(X_b{t+1}(1:2)) - 9];
        
        % Cost function design.
        f_a = f_a + ...
            0*(U_a{t}(1:2)'*U_a{t}(1:2)) + ... % Minimize acceleration
            1*(X_a{t+1}(1:2)'*X_a{t+1}(1:2));
        f_b = f_b + ...
            0*(U_b{t}(1:2)'*U_b{t}(1:2)) + ...
             1*(X_b{t+1}(1:2) - X_a{t+1}(1:2))' * (X_b{t+1}(1:2) - X_a{t+1}(1:2));
    end
end

slacks_a =  MX.sym('slacks_a', size(g_a,1));
slacks_b =  MX.sym('slacks_b', size(g_b,1));
