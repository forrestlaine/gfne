...
[Xp,z,duration,eps_optp] = ai_solver(@f, h, g, l, n, m, N, T, 1, x0,z);
duration
...
function val = running_cost_a_polite(xu)
    ...
    yc = xu(10);
    yd = xu(14);
    val = 1*ua'*ua + probc*running_cost_c(xu) + (1-probc)*running_cost_d(xu);
end
...
function val = running_cost_c(xu)
    ...
    xc = xu(9:10);
    xd = xu(13:14);
    const_ab = (xa-xb)'*(xa-xb);
    const_ac = (xa-xc)'*(xa-xc);
    const_ad = (xa-xd)'*(xa-xd);
%    
%     val_on = val - sqrt(const_ac);
%     val_off = val;
%     val = if_else(const_ac < sq_dist+2, val_on, val_off);
end
...
function val = running_cost_d(xu)
    ...
    xd = xu(13:14);
    const_ad = (xa-xd)'*(xa-xd);
%     val_on = val + (sq_dist+2-const_ad);
%     val_off = val;
%     val = if_else(const_ad < sq_dist+2, val_on, val_off);
end
...
function next = f(xu)
    ...
    za = xa + dt*dubins_dyn(xa,ua);
    zb = xb + dt*dubins_dyn(xb,ub);
    zc = xc + dt*dubins_dyn(xc,uc);
    zd = xd + dt*dubins_dyn(xd,ud);
