...
for t = 1:T
    ...
    U_X{t} = MX.sym(['U_' num2str(t)], 2);
    U_Y{t} = MX.sym(['U_' num2str(t)], 2);
    X{t+1} = MX.sym(['X_' num2str(t+1)], 4);
    Y{t+1} = MX.sym(['Y_' num2str(t+1)], 4);

    vars_x = [vars_x; U_X{t}; X{t+1}];
    vars_y = [vars_y; U_Y{t}; Y{t+1}];
