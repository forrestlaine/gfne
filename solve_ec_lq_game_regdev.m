...
function [dX,dU,dL,dM,dP,dD] = solve_ec_lq_game_regdev(F,... % time-indexed dict of dynamics
    ...
    for t = T-1:-1:1
        ...
        top_block = zeros(all_m+all_lh+n+ N*(n+all_m)+all_m,all_m+all_lh+N*n+N*all_m+n+all_m+size(M{t+1},2));
        bleft_block = zeros(size(M{t+1},1), all_m+all_m+all_lh+N*n+N*all_m+n);
        ...
        top_block(ind+1:ind+n,all_m+all_lh+n*N+N*all_m+all_m+1:all_m+all_lh+n*N+n+N*all_m+all_m) = -eye(n);
        ind = ind+n;
    
        ind2 = all_m+all_lh+N*n+N*all_m+n+all_m;
        ind3 = 0;
        ind4 = 0;
        ind5 = 0;
        ...
        for i = 1:N
            ...
            Ik = [Ik(1:start_m,:); Ik(start_m+m{i}+1:end,:)];
            ...
            top_block(ind+1:ind+n,ind2-n-N*all_m-N*n+1+(i-1)*n-all_m:ind2-n-N*all_m-N*n+i*n-all_m) = -eye(n);

            top_block(ind+1:ind+n,ind2-n-N*all_m-all_m+1+ind4:ind2-n-N*all_m+ind4) = KK';
            top_block(ind+n+1:ind+n+all_m-m{i},ind2-n-N*all_m-all_m+1+ind4:ind2-n-N*all_m+ind4) = -Ik;
        ...
        top_block(ind+1:ind+all_m, ind2-n-all_m+1:ind2-n) = reg*eye(all_m);
        top_vec(ind+1:ind+all_m) = reg*all_dev;
        ind4 = 0;
        ind5 = 0;
        for i = 1:N
            top_block(ind+ind5+1:ind+ind5+m{i},ind2-n-N*all_m-all_m+1+ind4+ind5:ind2-n-N*all_m-all_m+ind4+ind5+m{i}) = eye(m{i});
            bleft_block(ind5+1:ind5+m{i},end-n-all_m-N*all_m+ind4+ind5+1:end-n-all_m-N*all_m+ind4+ind5+m{i}) = -eye(m{i});
            ind4 = ind4+all_m;
            ind5 = ind5+m{i};
        end
        ind = ind+all_m;
    ...
    top_block = zeros(N*all_m+n+all_m, n+N*(all_m+n)+all_m+size(M{1},2));
    bleft_block = zeros(size(M{1},1), n+N*(all_m+n)+all_m);
    bleft_block(:,end-n+1:end) = Mx{1};
    top_vec = zeros(N*all_m+n+all_m,1);
    
    
    
    %%
    ind = 0;
    ind2 = n+N*(all_m+n)+all_m;
    ind3 = 0;
    ind4 = 0;
    ind5 = 0;
    all_KK = [];
    ...
    for i = 1:N
        ...
        top_block(ind+1:ind+n,ind2-n-N*all_m-all_m+1+ind4:ind2-n-N*all_m+ind4) = all_KK';
        top_block(ind+n+1:ind+n+all_m-m{i},ind2-n-N*all_m-all_m+1+ind4:ind2-n-N*all_m+ind4) = -Ik;
        top_block(ind+1:ind+n,ind2+all_m+ind5+1:ind2+all_m+ind5+lg{t,i}) = G{i}(:,2:1+n)';
        top_block(ind+1:ind+n,ind2+all_m+all_lg+(i-1)*n+1:ind2+all_m+all_lg+i*n) = Ftn(:,2:n+1)';
        top_block(ind+n+1:ind+(n+all_m-m{i}),ind2+all_m+all_lg+(i-1)*n+1:ind2+all_m+all_lg+i*n) = [Ftn(:,n+2:n+1+ind3)';Ftn(:,n+2+m{i}+ind3:end)'];
